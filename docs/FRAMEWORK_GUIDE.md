...
- **No `config.py`/settings module exists.** `chunk_size` (1000) and `chunk_overlap` (200) are each independently redeclared as default parameters in *two* files (`src/embedding.py`, `src/vectorstore.py`); `embedding_model` (`"all-MiniLM-L6-v2"`) is redeclared in *three* (those two plus `src/search.py`, which doesn't take chunk_size/chunk_overlap params at all — it only forwards `persist_dir`/`embedding_model` to `FaissVectorStore` and relies on that class's own chunk_size/chunk_overlap defaults). Changing embedding model means editing three places, chunk size/overlap means editing two, and keeping them in sync by hand either way.
