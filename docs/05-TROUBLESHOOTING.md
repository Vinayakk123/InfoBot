...
**Fix:** Constrain the prompt to answer only from retrieved context and explicitly instruct a fallback response when the answer isn't present. The current wording, in the answer-generation prompt inside `search_and_summarize_with_sources()` ([src/search.py:203-213](../src/search.py#L203-L213)), is: "Answer the current question using only the provided context... If the context does not contain the answer, say so." This was one of the most impactful changes for keeping answers grounded. (Retrieval itself already short-circuits to the literal fallback string `"No relevant documents found."` when no chunks are retrieved at all — see [src/search.py:189-196](../src/search.py#L189-L196) — so the prompt-level instruction only matters when context was retrieved but doesn't contain the answer.)
...
2. `search_and_summarize_with_sources()` ([src/search.py:134](../src/search.py#L134)) now accepts a `history` argument and uses the already-resolved query (not the raw one) in the final answer prompt, alongside the history — this fixed answers to 4/4 stable and correct in isolated testing, vs. intermittent hedging before. The prompt makes the grounding constraint explicit: history is only for interpreting the question and "is not a source of facts and must not be used to answer."
...
**Symptom:** After the fix above shipped, the opposite problem showed up: standalone questions on a new topic, and greetings/small talk sent after an earlier topic was discussed, were also getting rewritten by `_condense_query()` to relate to the previous topic — instead of being left alone — producing a nonsensical retrieval query and a wrong or hedging answer even though the new message didn't reference the prior turn at all.

**Cause:** The original condensation prompt only instructed the model to "rewrite the follow-up question as a standalone question that includes any context needed" and to return it unchanged "if the follow-up question is already standalone," without spelling out what actually counts as needing context. With no concrete rule or examples to anchor it, the small 8B model defaulted to relating every follow-up to the previous topic, including messages with no pronoun or implicit reference at all.

**Fix:** Rewrote the prompt in `_condense_query()` ([src/search.py:52-117](../src/search.py#L52-L117)) with an explicit rule: only rewrite when the new message contains a pronoun/reference word ("he", "she", "it", "they", "that", "this") or clearly continues the previous topic — otherwise copy the new message unchanged, even for greetings or a new question on the same general subject. Added three few-shot examples covering the rewrite case and both non-rewrite cases (greeting; unrelated new question), plus an output-format constraint (plain text, one sentence, no markdown or explanation) to stop the model from wrapping its reply in commentary.
